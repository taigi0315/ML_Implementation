...
#need library(plyr)
#need library(arules)
freq_discretize <- function(dat, freq){
  ...
  if(length(d_column) > 0){
    for(i in 1:length(d_column)) {
      if(i == 1){
        discretize_set <- matrix(discretize(o_data[,i],  "frequency", categories=freq, ordered = TRUE, onlycuts = TRUE), nrow=1)
        o_data[ ,d_column[i]] <- discretize(o_data[,i],  "frequency", categories=freq, ordered = TRUE)
      }
      else{
        discretize_set <- rbind(discretize_set, matrix(discretize(o_data[,i],  "frequency", categories=freq, ordered = TRUE, onlycuts = TRUE), nrow=1))
        o_data[ ,d_column[i]] <- discretize(o_data[,i],  "frequency", categories=freq, ordered = TRUE)
      }
    }
    
    for(i in 1:as.numeric(length(d_column))){#for each column that has factor data
      if(i == 1){
        o_data <- o_data[order(o_data[,i]),]
        category_f <- matrix(unique(o_data[,d_column[i]]), nrow = 1)
      } 
      else{
        o_data <- o_data[order(o_data[,i]),]
        category_f <- rbind.fill.matrix(category_f, matrix(unique(o_data[,d_column[i]]), nrow = 1))
      }
    }
    rownames(category_f) <- d_column
    write.csv(discretize_set, file = "f_discretize_set.csv")
    #change "fact" to character
    o_data[d_column] <- lapply(o_data[d_column], as.character)
    for(i in 1:length(d_column)){
      for(j in 1:freq){
        o_data[which(o_data[,i] == category_f[i,j]), i] <- (j-1)
      }
    }
  }
